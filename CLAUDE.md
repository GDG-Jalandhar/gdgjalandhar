...
```bash
pnpm dev        # dev server — runs against msw fixtures (src/mocks/), not the live API
pnpm build      # production build — hits the REAL Bevy API for generateStaticParams
pnpm start
pnpm lint       # eslint (flat config, eslint.config.mjs)
pnpm typecheck  # tsc --noEmit
pnpm test       # vitest run
pnpm test:watch
pnpm test:e2e   # playwright test — smoke + accessibility, runs against `pnpm dev`
pnpm test:e2e:offline  # offline-navigation.spec.ts only — builds + runs `pnpm start` (needs a real prod build; see Testing below)
```

## The `.next` cache gotcha — read this before debugging "my mock change did nothing"

`pnpm build` fetches from the **real** Bevy API (chapter 781, which currently returns `count: 0` — a real, current, empty state, not a bug). Next's on-disk fetch/data cache in `.next/cache/` is keyed by request URL+options, not by which command populated it. If you run `pnpm build` and then go back to `pnpm dev`, dev will silently serve the **stale real-API response** instead of hitting `msw`, because the cache doesn't know the underlying handler changed. Symptom: pages that worked fine now show empty states, and nothing in the logs looks wrong. Same thing happens after editing `src/mocks/handlers.ts` or fixture data — the old cached response for that URL is still on disk.
...
- **`src/mocks/`** — `msw` handlers + fixtures (`fixtures/events.ts`, `fixtures/agenda.ts`) used by both `next dev` (via `src/instrumentation.ts`, dev-only) and Vitest. The fixtures mirror real edge cases: a cohosted foreign-chapter event (`chapter_id: 887`, the PRD's own "tripwire" for mock data leaking into prod), a 13-row agenda, attendees exceeding capacity, a default-banner placeholder, and a malformed-agenda string with the exact invalid escapes real Bevy data contains.
...
- **Images**: `next/image`'s own optimizer handles responsive sizing from a single source URL — deliberately *not* hand-building a Cloudinary `c_scale,w_2560` transformation chain (PRD §6.6 D-6's suggested approach), since that risks the exact "malformed chain 404s" failure the PRD itself warns about. `EventBanner.tsx` centralizes the one thing D-6 does still require: detecting `GDG_Bevy_DefaultEventBanner` in the URL and substituting a branded fallback instead of Bevy's generic stock photo.
